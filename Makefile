...
build_uri_js:
	cat vendor/uri.js/build/uri.js | sed -e 's/window.URI/window.SC.URI/g' >> $(BUILD_DIR)/sdk.unminified.js
...
build_coffee:
	node_modules/coffee-script/bin/coffee --join $(BUILD_DIR)/sdk.unminified.js --compile src/*.coffee src/sc/*.coffee
